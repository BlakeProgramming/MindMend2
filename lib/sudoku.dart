...
class _SudokuScreenState extends State<SudokuScreen> {
  ...
  List<List<Set<String>>> notes = List.generate(9, (_) => List.generate(9, (_) => {}));
  List<List<bool>> fixedCells = List.generate(9, (_) => List.generate(9, (_) => false));
  int? selectedRow;
  int? selectedCol;
  ...
  void generatePuzzle() {
    List<List<String>> newBoard = List.generate(9, (_) => List.generate(9, (_) => ''));
    List<List<int>> grid = List.generate(9, (_) => List.generate(9, (_) => 0));

    fillDiagonal(grid);

    if (solveSudoku(grid)) {
      newBoard = grid.map((row) => row.map((e) => e.toString()).toList()).toList();
  ...
  void selectNumber(String number) {
    setState(() {
      ...
      if (selectedRow != null && selectedCol != null && !fixedCells[selectedRow!][selectedCol!]) {
        if (pencilMode) {
      ...
      body: Center(
        child: Column(
          ...
          children: [
            ...
            Text("Sudoku", style: TextStyle(color: Colors.white70, fontSize: 18)),
            SizedBox(height: 20),
            Container(
              width: gridSize,
              height: gridSize,
                ...
                children: List.generate(9, (row) {
                  return Row(
                    children: List.generate(9, (col) {
                      ...
                      bool isConflictCell = board[row][col] != '' && isConflict(row, col, board[row][col]);
                      return GestureDetector(
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                            border: Border(
                              top: BorderSide(color: Colors.black, width: row % 3 == 0 ? 1.5 : 0.5),
                              left: BorderSide(color: Colors.black, width: col % 3 == 0 ? 1.5 : 0.5),
                              right: BorderSide(color: Colors.black, width: (col + 1) % 3 == 0 ? 1.5 : 0.5),
                              bottom: BorderSide(color: Colors.black, width: (row + 1) % 3 == 0 ? 1.5 : 0.5),
                            ),
                            color: isConflictCell
                                ? Colors.red[100]
                                : isSelected
                                    ? Colors.purple[100]
                                    : Colors.white,
                              ...
                              Center(
                                child: Text(
                                  ...
                                  style: TextStyle(
                                    ...
                                    color: fixedCells[row][col] ? Colors.black : Colors.blue,
                                  ),
                                ),
                              ),
                              if (board[row][col] == '' && notes[row][col].isNotEmpty)
                                Padding(
                                  ...
                                  child: Align(
                                    ...
                                    child: Wrap(
                                      spacing: 2,
                                      runSpacing: 2,
                                      children: notes[row][col].map((note) {
                                        return Text(
                                          note,
              ...
              children: List.generate(9, (i) {
                ...
                return GestureDetector(
                  ...
                  child: Container(
                    ...
                    child: Text(num, style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                  ),
                );
              }),
            ),
            ...
            Row(
              ...
              children: [
                IconButton(
                  icon: Icon(LucideIcons.pencil, color: pencilMode ? Colors.purple[200] : Colors.white),
